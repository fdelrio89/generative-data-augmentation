...
from data.augmented_flickr30k_dataset import augmented_flickr30k
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    elif dataset=='caption_augmented_flickr':
        # train_dataset = flickr30k_train(transform_train, config['image_root'], config['ann_root'], prompt=config['prompt'])
        # val_dataset = coco_karpathy_caption_eval(transform_test, config['image_root'], config['ann_root'], 'val')
        # test_dataset = coco_karpathy_caption_eval(transform_test, config['image_root'], config['ann_root'], 'test')
        # return train_dataset, val_dataset, test_dataset
        path_text_data = config['ann_root']
        list_skills = config['list_skills'].split(',')
        image_roots_dict = config['image_root']
        transform_list = {
            'train': transform_train,
            'val': transform_test,
            'test': transform_test,
            **{f'train_{skill}': transform_train for skill in list_skills},
            **{f'test_{skill}': transform_test for skill in list_skills}
        }
        prompt = config['prompt']
        image_ext_dict = config['image_exts']

        return augmented_flickr30k.build_all(
            path_text_data,
            image_roots_dict=image_roots_dict,
            transforms=transform_list,
            prompt=prompt,
            list_skills=list_skills,
            image_ext_dict=image_ext_dict,
        )
