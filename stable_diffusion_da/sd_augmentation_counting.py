...
def generate_images(
    ...
    for i in range(num_iterations):
        ...
        for idx, image in enumerate(images.images):
            image_name = f"images/image_{(i*num_images_per_prompt)+idx}_{image_id}_counting.png"
            image.save(image_name)
            if display_images:
                display(Image(filename=image_name, width=128, height=128))

df_gender_captions = pd.read_csv('data/Image_quality_testing_counting.tsv', sep='\t')
